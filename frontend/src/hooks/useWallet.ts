...
export function useWallet(): WalletState {
  ...
  const connect = useCallback(async () => {
    const eth = getEth();
    if (!eth) {
      setError('No wallet detected. Install a GenLayer compatible wallet to take the bench.');
      return;
    }
    setConnecting(true);
    setError(null);
    try {
      ...
      if (/reject|denied|4001/i.test(msg)) setError('You declined the connection');
